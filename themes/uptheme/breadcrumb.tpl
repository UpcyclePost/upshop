...
{if $page_name != 'product' && $page_name !='order-opc' && $page_name !='order' && $page_name !='order-confirmation' && $page_name !='address'}
<!-- Breadcrumb -->
...
<div class="breadcrumb clearfix">
	...
	{if isset($path) AND $path}
		...
		{if $path|strpos:'span' !== false}
			<span class="navigation_page">{$path|@replace:'<a ': '<span itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" '|@replace:'data-gg="">': '><span itemprop="title">'|@replace:'</a>': '</a></span>'}</span>
		{else}
			{$path}
		{/if}
	{/if}
