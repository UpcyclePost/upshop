<?php
	 ...
	 */
		...
		public function getAllRangeAccordingToShippingId() {
			$is_range = Db::getInstance()->executeS("select `id` as id_range,delimiter1,delimiter2 from "._DB_PREFIX_."mp_range_weight where mp_shipping_id=".$this->mp_shipping_id);
			if(empty($is_range)) {
				return false;
			} else {
				return $is_range;
		...
		public function isRangeInTableByShippingId() {
			$is_range = Db::getInstance()->executeS("select * from "._DB_PREFIX_."mp_range_weight where mp_shipping_id=".$this->mp_shipping_id." and delimiter1=".$this->delimiter1." and delimiter2=".$this->delimiter2);
			if(empty($is_range)) {
				return false;
			} else {
				return $is_range;
		...
		public function findRangeIdBetweenDelimetr($mp_shipping_id,$weight) {
			$mp_range_id = Db::getInstance()->getRow("select `id` from `"._DB_PREFIX_."mp_range_weight` where mp_shipping_id=".$mp_shipping_id." and ".$weight." BETWEEN  `delimiter1` AND  `delimiter2` ");
