		...
		$(".buttonNext").click(function(e){
			...
			else
			{
				if(rel==1)
				{
					...
					$('#selectedstep1').attr('class','done');
					$('#selectedstep1 a').attr('class','done');
					$('#selectedstep2').attr('class','selected');
					$('#selectedstep2 a').attr('class','selected');
				...
				else if(rel==2)
				{
					...
					if(shipping_charge_error_message == "")
					{
						...
						$('#selectedstep2').attr('class','done');
						$('#selectedstep2 a').attr('class','done');
						$('#selectedstep3').attr('class','selected');
						$('#selectedstep3 a').attr('class','selected');
	...
	$("#shipping_logo").change(function(){
		...
		$('.buttonPrevious').click(function(e) {
			...
			if(rel==2) {
				...
				$('#selectedstep1').attr('class','selected');
				$('#selectedstep1 a').attr('class','selected');
				
				$('#step-1').css('display','block');
				$('#step-2').css('display','none');
				$('.buttonPrevious').addClass('buttonDisabled');
			} else if(rel==3) {
				$('#selectedstep3').attr('class','done');
				$('#selectedstep3 a').attr('class','done');
		...
		$('.buttonFinish').live('click', function (e) {
			
			var class_name = $(this).attr('class');
			if(class_name=='buttonFinish') 
			{
				//alert($('#step_carrier_general').attr('action'));
				//e.preventDefault();
				submitForm();
...
function displayRangeType()
{
	...
	else if (shipping_method == 2 || shipping_method == 3)
	{
		string = string_price;
		$('.price_unit').show();
		$('.weight_unit').hide();
...
function add_new_range()
{
	...
	$('tr.range_sup td:last').after('<td class="center"><div class="input-group fixed-width-md"><span class="input-group-addon price_unit" style="display: none;">&nbsp; '+currency_sign+'</span><input name="range_sup[]" class="form-control" type="text" /><span class="weight_unit" style="display: none;">&nbsp; '+PS_WEIGHT_UNIT+'</span></div></td>');
	...
	$('tr.fees_all td:last').after('<td class="center border_top border_bottom"><div class="input-group fixed-width-md"><span class="input-group-addon currency_sign" style="display:none" >&nbsp;'+currency_sign+'</span><input style="display:none" class="form-control" type="text" /></div></td>');

	$('tr.fees').each( function () {
		$(this).children('td:last').after('<td class="center"><div class="input-group fixed-width-md"><span class="input-group-addon" >&nbsp;'+currency_sign+'</span><input disabled="disabled" class="form-control" name="fees['+$(this).data('zoneid')+'][]" type="text" /></div></td>');
	});
...
function editcheckAllZones(elt)
{
	if($(elt).is(':checked'))
	{
		$('.input_zone').attr('checked', 'checked');
...
function editenableTextField(checkbox_obj)
{
...
$(document).ready(function(){
	$('#edit_shipping_method_price').on('click', function() { //alert("hello");
		$('#zone_ranges .range_type').text('Will be applied when the price is');
		$('#zone_ranges .edit_price_sign').removeAttr( 'style' );
		$('#zone_ranges .edit_weight_sign').attr( 'style', 'display:none;' );
	});

	$('#edit_shipping_method_weight').on('click',function(){
		$('#zone_ranges .range_type').text('Will be applied when the weight is');
		$('#zone_ranges .edit_weight_sign').removeAttr( 'style');
		$('#zone_ranges .edit_price_sign').attr( 'style', 'display:none;' );
	});
});
$(document).ready(function(){
	...
	window.onload = checkstatusofhandlingcharge;
	function checkstatusofhandlingcharge()
	{
		if($("#shipping_handling_on").is(":checked"))
			$("#shipping_handling_charge").show();
		else
