<script>
	...
	var gamification_level = '{l s='Level' js=1 mod='marketplace'}';
</script>
<style>
	{literal}.gamification_progress-label {top:-1px!important;-moz-border-radius: 15px;-o-border-radius: 15px;-webkit-border-radius: 15px;border-radius: 15px;height:19px;background-image: url(../modules/gamification/views/img/gamification-bar-bg.png) ;width: {/literal}{$current_level_percent|escape:'html':'UTF-8'}{literal}%}{/literal}
</style>
<div id="gamification_notif" class="notifs">
	...
	<div id="gamification_notif_wrapper" class="notifs_wrapper" style="width:340px">
		<div id="gamification_top">
			<h3>{l s='Your Merchant Expertise' mod='marketplace'}</h3>
		</div>
		<span style="font-size: 15px;color: #585A69;text-shadow: 0 1px 0 #fff;">{l s='Level' mod='marketplace'} {$current_level|intval} : {$current_level_percent|intval} %</span>
			...
			<ul id="gamification_badges_list" style="{if $badges_to_display|count <= 2} height:140px;{/if}">
				...
				{if $badge->id}
					<li class="{if $badge->validated} unlocked {else} locked {/if}" style="float:left;">
						...
						<div class="gamification_badges_title"><span>{if $badge->validated} {l s='Last badge :' mod='marketplace'} {else} {l s='Next badge :' mod='marketplace'} {/if}</span></div>
						<div class="gamification_badges_img"><img src="{$badge->getBadgeImgUrl()|escape:'html':'UTF-8'}"></div>
		...
		<a id="gamification_see_more" href="{$link->getAdminLink('AdminGamification')|escape:'html':'UTF-8'}">{l s='View my complete profile' mod='marketplace'}</a>
	</div>
</div>
