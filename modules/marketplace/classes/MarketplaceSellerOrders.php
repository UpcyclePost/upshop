...
class MarketplaceSellerOrders extends ObjectModel
{
	...
	public function getOrderShipping($id_order)
	{
		$shipping = Db::getInstance()->getValue('SELECT `total_shipping_tax_incl` FROM `'._DB_PREFIX_.'orders` where `id_order`='.(int)$id_order);
		if ($shipping)
			return $shipping;
		else
			return 0;
