	...
	class MarketplaceShop extends ObjectModel
	{
		...
		public static $definition = array(
			...
			'fields' => array(
				...
				'shop_name' => array('type' => self::TYPE_STRING, 'validate' => 'isCatalogName', 'required' => true, 'size' => 128),
		...
		{
			$mp_seller_id = Db::getInstance()->executeS("SELECT mc.`marketplace_seller_id`
												FROM "._DB_PREFIX_."marketplace_shop AS ms
												LEFT JOIN "._DB_PREFIX_."marketplace_customer AS mc
												ON (ms.`id_customer` = mc.`id_customer`)
												WHERE ms.`id`=".$id);
