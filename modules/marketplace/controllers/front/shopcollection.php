    ...
    public function initContent() 
    {
        ...
        if ($marketplace_shop_product) 
        {
			...
			if(Tools::getIsset('cat_id'))
            {
				...
				for ($i = 0; $i < $count; $i++) 
                {
                    $id_product = Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'category_product` WHERE `id_category` = '.Tools::getValue('cat_id'));
				...
				$this->context->smarty->assign("cat_id",Tools::getValue('cat_id'));
			}
            else
