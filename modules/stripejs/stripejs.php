...
class StripeJs extends PaymentModule
{
	...
	public function hookPayment($params)
	{
		...
		return '<script type="text/javascript" src="'. $this->_path .'js/stripe-prestashop.js"></script>
		<script type="text/javascript">'.
	...
	public function hookBackOfficeHeader()
	{
		...
		{
			/* Get transaction details and make sure the token is valid */
			$stripe_transaction_details = Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'stripe_transaction WHERE id_order = '.(int)Tools::getValue('id_order').' AND type = \'payment\' AND status = \'uncaptured\'');
			if (isset($stripe_transaction_details['id_transaction']) && $stripe_transaction_details['id_transaction'] === Tools::getValue('id_transaction_stripe'))
			{
		...
		if (Db::getInstance()->getValue('SELECT module FROM '._DB_PREFIX_.'orders WHERE id_order = '.(int)Tools::getValue('id_order')) == $this->name)
		{
			...
			$stripe_transaction_details = Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'stripe_transaction WHERE id_order = '.(int)Tools::getValue('id_order').' AND type = \'payment\' AND status IN (\'paid\',\'uncaptured\')');
			...
			if (!empty($stripe_transaction_details['id_transaction']))
				...
				$output .= '<b style="color: #CC0000;">'.$this->l('Warning:').'</b> '.$this->l('The customer paid using Stripe and an error occured (check details at the bottom of this page)');
				 ...
				 $output .= ((empty($this->_errors['stripe_capture_error']) && Tools::getIsset('id_transaction_stripe') && Tools::getIsset('SubmitStripeCapture')) ? '<div class="conf confirmation">'.$this->l('Your capture was successfully processed').'</div>' : '').
			(!empty($this->_errors['stripe_capture_error']) ? '<div class="conf confirmation">'.$this->l('Error:').' '.Tools::safeOutput($this->_errors['stripe_capture_error']).'</div><br /><br />' : '');
				 }
           ...
           if($stripe_transaction_details['status'] == 'uncaptured'){
			...
			$output .= '</fieldset><br />\').appendTo(appendEl);
				});
			</script>';
				}else {
			...
			$output .= ((empty($this->_errors['stripe_refund_error']) &&  Tools::getIsset('id_transaction_stripe')) ? '<div class="conf confirmation">'.$this->l('Your refund was successfully processed').'</div>' : '').
			...
			$output .= $this->l('Already refunded:').' <b>'.Tools::displayPrice($stripe_refunded).'</b><br /><br />'.($stripe_refunded ? '<table class="table" cellpadding="0" cellspacing="0" style="font-size: 12px;"><tr><th>'.$this->l('Date').'</th><th>'.$this->l('Amount refunded').'</th><th>'.$this->l('Status').'</th></tr>'.$output_refund.'</table><br />' : '').
    ...
    public function processCapture($id_transaction_stripe, $amount)
	{
		...
		try
		{
			...
			$result_json = Tools::jsonDecode($charge->capture(array('amount' => $amount * 100)));
			if(@$result_json->captured==true || @$result_json->captured==1){
		    $query = 'UPDATE ' . _DB_PREFIX_ . 'stripe_transaction SET `status` = \'paid\', `amount` = ' . $amount . ' WHERE `id_transaction` = \''. pSQL($id_transaction_stripe).'\'';
	...
	public function processPayment($token)
	{
		...
		try
		{
			...
			' '.(int)$this->context->cookie->id_customer.' - '.$this->l('PrestaShop Cart ID:').' '.(int)$this->context->cart->id, 'capture' => Configuration::get('STRIPE_CAPTURE_TYPE'));
		...
		if (isset($result_json->id))
			...
			\''.($result_json->amount * 0.01).'\', \''.($result_json->paid == 'true' ? (Configuration::get('STRIPE_CAPTURE_TYPE') == 'false' ? 'uncaptured' : 'paid'): 'unpaid').'\', \''.pSQL($result_json->currency).'\',
	...
	public function getContent()
	{
		...
		<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
			<fieldset class="stripe-settings">
				...
				</select><div style="margin-left:255px">
				'.$this->l('Choose whether to authorize payments and manually capture them later, or to both authorize and capture (i.e. fully charge) payments when orders are placed. You can capture a payment that is only Authorized by using Stripe payment tab for the order.').'</div>
				<hr>
